...
export const ListingSlider = ({listingPhotosArray}) => {
    const scrollContainerRef = useRef(null);
    const itemWidth = 275 + 8 // item width + gap width
    ...
    const handlePrevPhoto = () => {
        console.log("handlePrevPhoto FIRED")
        const prevPhoto = activePhoto.id === 1 ? listingPhotosArray[0] : listingPhotosArray.find((photo) => photo.id === activePhoto.id - 1)
        setActivePhoto(prevPhoto)
    }

    const handleNextPhoto = () => {
        console.log("handleNextPhoto FIRED")
        const nextPhoto = activePhoto.id === listingPhotosArray.length ? listingPhotosArray[listingPhotosArray.length - 1] : listingPhotosArray.find((photo) => photo.id === activePhoto.id + 1)
        setActivePhoto(nextPhoto)
    }
            ...
            {photoZoomOpen && (
                <div className="fixed cursor-pointer top-0 left-0 w-full h-full z-50 bg-black/80 flex justify-center items-center overflow-y-hidden" data-role="modal" onClick={(e) => handleCloseZoomPhoto(e)}>
                    ...
                    <div className="relative">
                        ...
                        <RiArrowLeftSFill className={`z-10 absolute ${activePhoto.id === 1 && "hidden"} -left-3 top-1/2 -translate-y-1/2 cursor-pointer text-white/90 no-select`} size={80} onClick={handlePrevPhoto} />
                        <RiArrowRightSFill className={`z-10 absolute ${activePhoto.id === listingPhotosArray.length && "hidden"} -right-3 top-1/2 -translate-y-1/2 cursor-pointer text-white/90 no-select`} size={80} onClick={handleNextPhoto} />
                    </div>
                </div>
            )}
        </>
