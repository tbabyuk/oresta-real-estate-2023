...
export const Navbar = () => {
  ...
  return (
        <nav className="px-5 h-[44px] flex justify-center items-center bg-oresta-light relative">
            {dropdownIsOpen ? (
                <IoCloseOutline 
                ...
                <RxHamburgerMenu 
                className="absolute left-2 top-1.5 cursor-pointer md:hidden text-oresta-primary" size="2rem"
                onClick={() => setDropdownIsOpen(!dropdownIsOpen)}
                />
            ) }
            ...
            <ul className={`h-full md:flex md:flex-row md:justify-center md:static ${dropdownIsOpen ? "flex-col w-full absolute -bottom-11" : "hidden"} text-gray-800`}>
                ...
                <li className="relative"><Link href="#" className="w-full h-full flex justify-center items-center px-5 py-3 bg-oresta-light/80" onMouseEnter={() => setResourcesSubmenuOpen(true)} onMouseLeave={() => setResourcesSubmenuOpen(false)} onClick={() => setResourcesSubmenuOpen(!resourcesSubmenuOpen)}>RESOURCES <IoIosArrowDown className="ms-1" /></Link>
